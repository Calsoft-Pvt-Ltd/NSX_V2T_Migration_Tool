...
__all__ = [
    ...
    "SchamaMultipleError", # Added by calsoft to introduced new error type
]
...
class SchamaMultipleError(SchemaError):
    """ Error should be raised when user want to show consolidated errors."""
...
class Schema(object):
    ...
    def iter_error(self, error_message):
        """ A function to catch all error and iterate through the same"""
        yield error_message
    ...
    def validate(self, data, **kwargs):
        ...
        if flavor == DICT:
                        ...
                        try:
                            ...
                            if isinstance(skey, Hook):
                                ...
                                try:
                                    ...
                                    print(f"Message {message, x.autos}")
                                    err = [message] + x.autos
                                    Schema.error_message.extend(err)
                                    # self.iter_error(Schema.error_message.extend(err))
                                    logging.error([message] + x.autos, [e.format(data) if e else None] + x.errors)
            ...
            if not required.issubset(coverage):
                ...
                err = message
                Schema.error_message.append(err)
                # self.iter_error(Schema.error_message.append(err))
