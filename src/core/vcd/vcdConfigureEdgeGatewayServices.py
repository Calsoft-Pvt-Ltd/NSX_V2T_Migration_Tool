...
class ConfigureEdgeGatewayServices(VCDMigrationValidation):
    ...
    def cidrCalculator(self, rangeofips):
        ...
        try:
            ...
            listOfIpsInIpRange = [str(ipaddress.IPv4Address(ip)) for ip in
                                  range(int(ipaddress.IPv4Address(start)), int(ipaddress.IPv4Address(end) + 1))]
    ...
    def configureFirewall(self, networktype=False, configureIPSET=False):
        ...
        try:
            ...
            for sourceEdgeGateway in self.rollback.apiData['sourceEdgeGateway']:
                logger.debug(
                    "Configuring Firewall Services in Target Edge Gateway - {}".format(sourceEdgeGateway['name']))
                sourceEdgeGatewayId = sourceEdgeGateway['id'].split(':')[-1]
                edgeGatewayId = \
                list(filter(lambda edgeGatewayData: edgeGatewayData['name'] == sourceEdgeGateway['name'],
                ...
                if not networktype:
                    ...
                    if response.status_code == requests.codes.ok:
                        ...
                        raise Exception(
                            "Failed to retrieve application port profiles - {}".format(response['error']['details']))
                        ...
                        if firewallRule.get('source', None):
                                    ...
                                    for ipsetgroup in ipsetgroups:
                                        ...
                                        if ipsetresponse.status_code == requests.codes.ok:
                                            ...
                                            if firewallIdDict.get(edgeGatewayId):
                                                ...
                                                if firewallIdDict[edgeGatewayId].get(
                                                        ipsetresponseDict['ipset']['name']):
                                                    ipsetDict = firewallIdDict[edgeGatewayId][
                                                        ipsetresponseDict['ipset']['name']]
                                            ...
                                            raise Exception(
                                                "Failed to retrieve ipset group {} info - {}".format(ipsetgroup,
                                                                                                     ipsetresponse[
                                                                                                         'error'][
                                                                                                         'details']))
                            ...
                            if networkgroups and networktype:
                                ...
                                if len(networkgroups) != 0 and firstTime:
                                    logger.debug(
                                        'Configuring security groups in the firewall in Target Edge Gateway - {}'.format(
                        ...
                        if not networktype:
                            ...
                            if response.status_code == requests.codes.accepted:
                                ...
                                if len(fwSourceScopeSet) == 1 and None not in fwSourceScopeSet:
                                    if not fwDestinationScopeSet or fwSourceScopeSet == fwDestinationScopeSet or (
                                            len(fwDestinationScopeSet) == 1 and None in fwDestinationScopeSet):
                                ...
                                if len(fwDestinationScopeSet) == 1 and None not in fwDestinationScopeSet:
                                    if not fwSourceScopeSet or fwSourceScopeSet == fwDestinationScopeSet or (
                                            len(fwSourceScopeSet) == 1 and None in fwSourceScopeSet):
                                        fwScopeDict[edgeGatewayId][firewallRule["id"]] = fwDestinationMappedToNat
                                self.rollback.apiData['fwScopeDict'] = fwScopeDict
                            else:
                                # failure in configuration of firewall rules on target edge gateway
                                response = response.json()
                                raise Exception('Failed to create Firewall rule on target Edge gateway {} - {}'.format(
                                    sourceEdgeGateway['name'], response['message']))
                    if not networktype:
                        logger.debug(
                            f"Firewall rules configured successfully on target Edge gateway {sourceEdgeGateway['name']}")
                    if not firstTime:
                        logger.debug(
                            f"Successfully configured security groups for Edge gateway {sourceEdgeGateway['name']}")
        except Exception:
    ...
    def checkNatIpFirewallMatching(self, sourceEdgeGatewayId, targetEdgeGatewayId, userDefinedFirewallRules, natRules):
        ...
        NonDistributedNetWorkList = [targetnetwork for targetnetwork in orgvdcNetworks for _, network in
                                     self.rollback.apiData["targetOrgVDCNetworks"].items()
                                     if network["networkType"] == "NAT_ROUTED" and
                                     network.get("connection", {}).get("routerRef", {}).get(
                                         "id") == targetEdgeGatewayId and
        ...
        for firewallRule in userDefinedFirewallRules:
            for natRule in listify(natRules.get('natRules', {}).get('natRule', [])):
                if natRule['action'] == 'dnat' and natRule['originalAddress'] in firewallRule.get('destination',
                                                                                                  {}).get('ipAddress',
                                                                                                          []):
                    if any([network for network in NonDistributedNetWorkList if
                            ipaddress.ip_address(natRule['translatedAddress'])
                            in ipaddress.ip_network('{}/{}'.format(network['subnets']['values'][0]['gateway'],
                                                                   network['subnets']['values'][0]['prefixLength']),
                                                    strict=False)]):
                        serviceList = listify(firewallRule.get("application", {}).get("service", []))
                        if any([natRule["protocol"] == service.get("protocol") and (
                                natRule["originalPort"] == service.get("port") or service.get("port") == "any")
    ...
    def getApplicationPortProfiles(self):
        ...
        try:
            ...
            while resultTotal > 0 and pageSizeCount < resultTotal:
                url = "{}{}?page={}&pageSize={}&filter=_context=={}&sortAsc=name".format(
                    vcdConstants.OPEN_API_URL.format(self.ipAddress),
        ...
        try:
            ...
            if value and len(value['applicationPorts'][0]['destinationPorts']) == 1:
                ...
                if response.status_code == requests.codes.accepted:
                    ...
                    applicationPortProfilesDict[
                        'TENANT-{}-{}-{}'.format(payloadDict['orgRef']['id'],
                                                 payloadDict['applicationPorts'][0]['protocol'],
                                                 payloadDict['applicationPorts'][0]['destinationPorts'][
                                                     0])] = payloadDict
    ...
    def configureTargetNAT(self):
        ...
        try:
            ...
            for sourceEdgeGateway in self.rollback.apiData['sourceEdgeGateway']:
                ...
                noSnatDestSubnet = self.orgVdcInput['EdgeGateways'][sourceEdgeGateway['name']][
                ...
                if not data or not data['enabled']:
                    logger.debug('NAT is not configured or enabled on Source Edge Gateway - {}'.format(
                        sourceEdgeGateway['name']))
                    return
                if data['natRules']:
                    ...
                    if data['enabled'] == 'true':
                        ...
                        for sourceNATRule in userDefinedNAT:
                            ...
                            if sourceNATRule['ruleId'] in rulesConfigured or sourceNATRule[
                                'ruleTag'] in rulesConfigured:
                            ...
                            payloadData = self.createNATPayloadData(sourceEdgeGatewayId, sourceEdgeGateway['name'],
                                                                    sourceNATRule, vNicsDetails,
                                                                    applicationPortProfilesList, version,
                                                                    defaultGatewayDict, destinationIpDict,
                                                                    noSnatRulesList,
                                                                    bgpConfigDetails, routingConfigDetails,
                                                                    noSnatDestSubnetList=noSnatDestSubnet)
                        ...
                        logger.debug(
                            'No NAT rules configured in Source Edge Gateway - {}'.format(sourceEdgeGateway['name']))
    ...
    def configStaticRoutes(self):
        ...
        for sourceEdgeGateway in self.rollback.apiData['sourceEdgeGateway']:
            ...
            edgeGatewayStaticRoutes = self.staticRouteCheck(sourceEdgeGatewayId, sourceEdgeGatewayName,
                                                            edgeGatewayStaticRouteDict, routeType='internal')
        ...
        try:
            ...
            for targetEdgeGateway in self.rollback.apiData.get('targetEdgeGateway', []):
                ...
                firewallUrl = "{}{}{}?pageSize=128".format(vcdConstants.OPEN_API_URL.format(self.ipAddress),
                                                           vcdConstants.ALL_EDGE_GATEWAYS,
                                                           vcdConstants.T1_ROUTER_FIREWALL_CONFIG.format(
                ...
                if response.status_code == requests.codes.ok:
                    ...
                    raise Exception(
                        "Failed to fetch target edge gateway {} firewall rules".format(targetEdgeGateway["name"]))
                # url to configure firewall rules on target edge gateway
                ...
                if response.status_code == requests.codes.ok:
                    ...
                    raise Exception(
                        "Failed to fetch target edge gateway {} NAT rules".format(targetEdgeGateway["name"]))
                ...
                logger.debug(
                    "Firewall rules scopes updated on target edge gateway {}".format(targetEdgeGateway["name"]))
        ...
        for sourceEdgeGateway in self.rollback.apiData['sourceEdgeGateway']:
            ...
            for network in self.retrieveNetworkListFromMetadata(sourceOrgVDCId):
                for subnet in network["subnets"]["values"]:
                    if network["networkType"] == "NAT_ROUTED" and network["connection"]["routerRef"]["id"].split(':')[
                        -1] == sourceEdgeGatewayId.split(':')[-1]:
                        allRoutedNetworkSubnets.append(
            ...
            for sourceEdgeGateway in self.rollback.apiData['sourceEdgeGateway']:
                ...
                if data:
                    ...
                    if apiResponse.status_code == requests.codes.accepted:
                        ...
                        logger.debug('DNS service configured successfully on target edge gateway - {}'.format(
                            sourceEdgeGateway['name']))
                        url = "{}{}{}".format(vcdConstants.OPEN_API_URL.format(self.ipAddress),
                                              vcdConstants.ALL_EDGE_GATEWAYS,
                                              vcdConstants.DNS_CONFIG.format(edgeGatewayID))
                        ...
                        if response.status_code == requests.codes.ok:
                            ...
                            logger.warning(
                                ...
                                " DNS server will be configured with this listener IP".format(
                                    responseDict['listenerIp']))
    ...
    def updateOrgVDCNetworkStaticIpPool(self, network, ipToBeRemove):
        ...
        if responsedict.get('mode') == "NETWORK" and responsedict.get('ipAddress'):
            logger.debug(
                "DHCP service already configured in network mode for OrgVDC network : {}.".format(network['name']))
            return
    ...
    def configureDHCPBindingService(self):
        ...
        for sourceEdgeGateway in self.rollback.apiData['sourceEdgeGateway']:
            ...
            if not DHCPData.get('staticBindings'):
                logger.debug(
                    "DHCP static bindings service not configured on source edge gateway : {}.".format(
                        sourceEdgeGateway))
                continue

            logger.debug(
                'Configuring DHCP static bindings service on target edge gateway - {}'.format(
                    sourceEdgeGateway['name']))
            ...
            for binding in staticBindings:
                ...
                if response.status_code == requests.codes.ok:
                    ...
                    response = response.json()
                    raise Exception("Failed to retrieve DHCP configuration info for network {} - {}".format(networkId,
                                                                                                            response[
                                                                                                                'message']))
    ...
    def configureDHCPRelayService(self):
        ...
        for sourceEdgeGateway in self.rollback.apiData['sourceEdgeGateway']:
            ...
            if apiResponse.status_code == requests.codes.accepted:
                ...
                errorResponse = apiResponse.json()
                raise Exception(
                    "Failed to configure DHCP forwarder on edge gateway {}, error : {}.".format(targetEdgeGatewayID,
                                                                                                errorResponse[
                                                                                                    'message']))
            ...
            for network in orgvdcNetworks:
                ...
                if apiResponse.status_code == requests.codes.accepted:
                    ...
                    errorResponse = apiResponse.json()
                    raise Exception(
                        "Failed to enable DHCP in relay mode on OrgVDC network {}, error : {}.".format(network['name'],
                                                                                                       errorResponse[
                                                                                                           'message']))
        ...
        try:
            ...
            for networkgroup in networkgroups:
                ...
                if getnetworkResponse.status_code == requests.codes.ok:
                    ...
                    raise Exception("Failed to retrieve network info for {} - {}".format(networkgroup,
                                                                                         getnetworkResponse['message']))
            ...
            for firewallGroupId in firewallGroupIds:
                ...
                if getGroupResponse.status_code == requests.codes.ok:
                    ...
                    raise Exception("Failed to retrieve firewall group {} info - {}".format(firewallGroupId,
                                                                                            getGroupResponse[
                                                                                                'message']))
                ...
                # else create the new firewall group
                    ...
                    for member in newMembers:
                        ...
                        if response.status_code == requests.codes.accepted:
                            ...
                            firewallGroupId = self._checkTaskStatus(taskUrl=taskUrl, returnOutput=True)
                            logger.debug(
                                'Successfully configured security group for firewall {}.'.format(firewallRule['id']))
                            ...
                            if putResponse.status_code == requests.codes.accepted:
                                ...
                                logger.debug(
                                    "Successfully added network '{}' to security group '{}'".format(member["name"],
                                                                                                    firewallGroupDict[
                                                                                                        'name']))
                            else:
        ...
        try:
            ...
            for bgpNeighbour in bgpNeighbours:
                ...
                if bgpNeighbour.get("bgpFilters"):
                    ...
                    if infilters:
                        inRoutesFilterRef = self.createBGPFilters(bgpFilters=infilters, edgeGatewayID=edgeGatewayID,
                                                                  filtertype='in', bgpNeighbour=bgpNeighbour)
                        bgpNeighbourpayloadDict['inRoutesFilterRef'] = inRoutesFilterRef if inRoutesFilterRef else ''
                    if outfilter:
                        outRoutesFilterRef = self.createBGPFilters(bgpFilters=outfilter, edgeGatewayID=edgeGatewayID,
                                                                   filtertype='out', bgpNeighbour=bgpNeighbour)
    ...
    def createNATPayloadData(self, sourceEdgeGatewayId, sourceEdgeGatewayName, sourceNATRule, vNicsDetails,
                             applicationPortProfilesList, version,
        ...
        if float(self.version) >= float(vcdConstants.API_VERSION_BETELGEUSE_10_4):
            for vnicData in vNicsDetails:
                if sourceNATRule["vnic"] == vnicData["index"]:
                    if "portgroupName" in vnicData and (
                            vnicData["portgroupName"] in self.rollback.apiData["sourceOrgVDCNetworks"] or \
                            vnicData["portgroupName"] in self.rollback.apiData.get("isT1Connected", {}).get(
                        sourceEdgeGatewayName, {})):
                        self.rollback.apiData["natInterfaces"][sourceEdgeGatewayId][sourceNATRule['ruleTag']] = \
                        vnicData["portgroupName"]
                    break
        ...
        if sourceNATRule['action'] == "dnat":
            ...
            if float(self.version) <= float(vcdConstants.API_VERSION_PRE_ZEUS):
                payloadData["internalPort"] = sourceNATRule['originalPort'] if sourceNATRule[
                                                                                   'originalPort'] != 'any' else ''
            else:
                payloadData["dnatExternalPort"] = sourceNATRule['originalPort'] if sourceNATRule[
                                                                                       'originalPort'] != 'any' else ''
        ...
        if sourceNATRule['action'] == "snat":
            ...
            if not payloadDict[
                'snatDestinationAddresses'] and ipInSuAllocatedStatus == False and destinationIpDict != {}:
                ...
                for ipsetgroup in ipsetgroups:
                    ...
                    if ipsetresponse.status_code == requests.codes.ok:
                        ...
                        if response.status_code == requests.codes.accepted:
                            ...
                            taskUrl = response.headers['Location']
                            firewallGroupId = self._checkTaskStatus(taskUrl=taskUrl, returnOutput=True)
                            self.rollback.apiData[
                                ipsetgroup['objectId'].split(':')[-1]] = 'urn:vcloud:firewallGroup:{}'.format(
                                firewallGroupId)
                        ...
                        raise Exception(
                            'Failed to get IPSET details due to error - {}'.format(errorResponse['message']))
    ...
    def getOrgVDCNetworksVNics(self, edgeGatewayId, orgvdcNetworkName):
        ...
        if response.status_code == requests.codes.ok:
            ...
            for edgeInterface in responseDict['edgeInterfaces']['edgeInterface']:
                ...
                raise Exception(
                    "Failed to get VNic details for edge gateway {} of network name {}.".format(edgeGatewayId,
                                                                                                orgvdcNetworkName))
        else:
            raise Exception(
                "Failed to get VNic details for edge gateway {} of network name {}.".format(edgeGatewayId,
    ...
    def dhcpRollBack(self, networkDisconnectedList=None):
        ...
        try:
            ...
            for sourceEdgeGatewayId in self.rollback.apiData['sourceEdgeGatewayId']:
                ...
                if data[sourceEdgeGatewayId]['ipPools'] or data[sourceEdgeGatewayId].get('staticBindings') or data[
                    sourceEdgeGatewayId].get('relay'):
                        ...
                        while timeout < vcdConstants.VCD_CREATION_TIMEOUT:
                            ...
                            if response.status_code == requests.codes.ok:
                                ...
                                if responseDict['edgeJob']['status'] == "FAILED":
                                    logger.debug("Failed configuring DHCP service in edge gateway {}".format(
                                        responseDict['edgeJob']['message']))
                    ...
                    if response.status_code == requests.codes.accepted:
                        ...
                        while timeout < vcdConstants.VCD_CREATION_TIMEOUT:
                            ...
                            if response.status_code == requests.codes.ok:
                                ...
                                # checking for the status of each polling call for 'Completed'
                                if responseDict['edgeJob']['status'] == 'COMPLETED':
                                    logger.debug('Rollback for IPSEC is completed successfully in {}'.format(
                                        sourceEdgeGateway['name']))
                                    break
                                    # checking if the task failed
                                if responseDict['edgeJob']['status'] == "FAILED":
                                    logger.debug("Failed configuring IPSEC VPN in edge gateway {}".format(
                                        responseDict['edgeJob']['message']))
                                    raise Exception(responseDict['edgeJob']['message'])
        except Exception:
            raise
        ...
        try:
            ...
            while resultTotal > 0 and pageSizeCount < resultTotal:
                ...
                if response.status_code == requests.codes.ok:
                    ...
                    raise Exception(
                        f'Failed to fetch load balancer virtual service details: {responseDict.get("message")}')
    ...
    def loadBalancerRollback(self):
        ...
        try:
            ...
            for edgeGateway in self.rollback.apiData['targetEdgeGateway']:
                logger.debug(
                    "Removing load balancer configuration from target edge gateway '{}'".format(edgeGateway['name']))
                edgeGatewayId = edgeGateway['id']
                ...
                for virtualService in virtualServices:
                    ...
                    if response.status_code == requests.codes.accepted:
                        ...
                        self._checkTaskStatus(taskUrl=taskUrl)
                        logger.debug(
                            f'Deleted virtual service {virtualService["name"]} from edge gateway {edgeGateway["name"]}')
                    else:
                    ...
                    if response.status_code == requests.codes.accepted:
                        ...
                        raise Exception(
                            "Failed to remove service engine group {} from edge gateway {} due to error {}".format(
                                serviceEngineGroup["serviceEngineGroupRef"]["name"], edgeGateway["name"],
    ...
    def configureLoadBalancer(self, nsxvObj):
        ...
        try:
            ...
            for sourceEdgeGateway in self.rollback.apiData['sourceEdgeGateway']:
                ...
                logger.debug(
                    "Configuring LoadBalancer Services in Target Edge Gateway - {}".format(sourceEdgeGateway['name']))
                serviceEngineGroupName = self.orgVdcInput['EdgeGateways'][sourceEdgeGateway['name']][
                ...
                self.createLoadBalancerPools(sourceEdgeGatewayId, sourceLbConfig, targetEdgeGatewayId,
                                             targetEdgeGatewayName, nsxvObj)
                self.createLoadBalancerVirtualService(sourceEdgeGatewayId, sourceLbConfig, targetEdgeGatewayId,
                                                      targetEdgeGatewayName)
    ...
    def createLoadBalancerVirtualService(self, sourceEdgeGatewayId, sourceLbConfig, targetEdgeGatewayId,
                                         targetEdgeGatewayName):
        """
        ...
        for virtualServer in virtualServersData:
            ...
            if virtualServer['name'] in [service['name'] for service in virtualServices]:
                logger.debug(
                    f'Virtual service {virtualServer["name"]} already created on target edge gateway {targetEdgeGatewayName}')
                if not virtualServer['retainSourceVip']:
    ...
    @isSessionExpired
    def createLoadBalancerPools(self, sourceEdgeGatewayId, sourceLbConfig, targetEdgeGatewayId, targetEdgeGatewayName,
                                nsxvObj):
        """
        ...
        objectIdsOfCertificates = [profile['clientSsl']['serviceCertificate'] for profile in applicationProfiles if
                                   profile.get('clientSsl')]
    ...
    def enableLoadBalancerService(
        ...
        if not rollback:
            ...
            if float(self.version) >= float(
                    vcdConstants.API_VERSION_BETELGEUSE_10_4) and LoadBalancerServiceNetworkIPv6:
    ...
    def enableDHCPv6InSlaccMode(self, targetEdgeGatewayId, targetEdgeGatewayName, rollback=False):
        ...
        try:
            if rollback:
                logger.debug(
                    'Disabling DHCPv6 service In Slacc Mode on target Edge Gateway-{} as a part of rollback'.format(
                        targetEdgeGatewayName))
            else:
                logger.debug(
                    'Enabling DHCPv6 service In Slacc Mode on target Edge Gateway-{}'.format(targetEdgeGatewayName))
    ...
    def assignServiceEngineGroup(self, targetEdgeGatewayId, targetEdgeGatewayName, serviceEngineGroupDetails):
        ...
        try:
            ...
            payloadDict = {
                ...
                "maxVirtualServices": serviceEngineGroupDetails['maxVirtualServices'] if serviceEngineGroupDetails[
                                                                                             'reservationType'] == 'SHARED' else None,
                "serviceEngineGroupRef": {
                    "name": self.serviceEngineGroupName,
                    "id": self.serviceEngineGroupId
                },
            ...
            if response.status_code == requests.codes.accepted:
                ...
                taskUrl = response.headers['Location']
                self._checkTaskStatus(taskUrl)
                logger.debug('Successfully assigned Service Engine Group on target Edge Gateway-{}'.format(
                    targetEdgeGatewayName))
            else:
                errorResponseData = response.json()
                raise Exception("Failed to assign Service Engine Group on target Edge Gateway-{} with error-{}".format(
                    targetEdgeGatewayName, errorResponseData['message']))
        except:
            raise
        ...
        def all_networks(entities):
            ...
            for entity in entities:
                if entity['type'] == 'Network':
                    ...
                    includeMembers = (
                        sourceGroup['member'] if isinstance(sourceGroup['member'], list) else [sourceGroup['member']])
                    ...
                    if targetNet.endswith("-v2t"):
                        ...
                        if targetNetMetadata["networkType"] == "OPAQUE":
                            ...
                            sourceToTargetOrgNetIds[
                                vcdObj.rollback.apiData["sourceOrgVDCNetworks"][targetNet[:-4]]["id"]] = \
                            targetNetMetadata["id"]
                    else:
                        sourceToTargetOrgNetIds[vcdObj.rollback.apiData["sourceOrgVDCNetworks"][targetNet]["id"]] = \
                        targetNetMetadata["id"]
            ...
            for l3rule in allLayer3Rules:
                ...
                if l3rule.get('services'):
                    if l3rule['services'].get('service'):
                        ...
                        # list instance of application services
                        firewallRules = l3rule['services']['service'] if isinstance(l3rule['services']['service'],
                                                                                    list) else [
                            l3rule['services']['service']]
                        # iterating over the application services
                        for applicationService in firewallRules:
                            # Collect pre-configured DFW objects.
                            for service in layer3AppServices:
                                if applicationService.get('protocolName'):
                                    if applicationService['protocolName'] == 'TCP' or applicationService[
                                        'protocolName'] == 'UDP':
                                    ...
                                    if service['layer'] == 'layer3' or service['layer'] == 'layer4':
                                        if applicationService['name'] != 'FTP' and applicationService['name'] != 'TFTP':
                                            if service['element']['applicationProtocol'] == 'TCP' or \
                                                ...
                                                protocol_name, port_id = self._searchApplicationPortProfile(
                                                    applicationPortProfilesDict,
                                                    service['element']['applicationProtocol'],
                                        ...
                                        if service['element']['applicationProtocol'] == 'ICMP':
                                            for value in applicationPortProfilesList:
                                                if value['name'] == service['name'] or value['name'] == service[
                                                    'name'] + ' Request':
                                                    applicationServicesList.append(
                                                        {'name': value['name'], 'id': value['id']})
                                    if service['layer'] == 'layer7':
                                        for contextProfile in allNetworkContextProfilesList.values():
                                            if service['element']['appGuidName'] == contextProfile['name'] or \
                                                    applicationService['name'] == contextProfile['name']:
                                                networkContextProfilesList.append(
                                                    {'name': contextProfile['name'], 'id': contextProfile['id']})
                        payloadDict['applicationPortProfiles'] = applicationServicesList
    ...
    def fetchFirewallGroupsByDCGroup(self):
        ...
        for group in firewallGroupsSummary:
            groupType = group['typeValue'] if float(self.version) >= float(vcdConstants.API_VERSION_ANDROMEDA) else \
            group['type']
                ...
                for targetNetwork in targetNetworks:
                    for sourceNetwork in sourceNetworks:
                        if sourceNetwork['name'] + '-v2t' == targetNetwork['name']:
                            ...
                            if response.status_code == requests.codes.ok:
                                responseDict = response.json()
                                if targetNetwork['networkType'] == 'ISOLATED' or sourceNetwork[
                                    "networkType"] == 'DIRECT':
                                    ...
                                    if rollback:
                                        ...
                                        if allLayer3Rules or sourceNetwork['shared']:
                                            ...
                                            if sourceNetwork["networkType"] == "DIRECT" and ownerRefID.get(
                                                ...
                                                responseDict['ownerRef'] = {
                                                    'id': list(ownerRefID.values())[0] if targetNetwork[
                                                                                              'id'] not in list(
                                                        ownerRefID.keys()) else ownerRefID[targetNetwork['id']]}
                                    ...
                                    if response.status_code == requests.codes.accepted:
                                        ...
                                        logger.debug('The network - {} scope has been changed successfully'.format(
                                            responseDict['name']))
                                    else:
                                        errorResponse = response.json()
                                        # failure in increase scope of the network
                                        raise Exception(
                                            'Failed to change scope of the network {} - {}'.format(responseDict['name'],
                                                                                                   errorResponse[
                                                                                                       'message']))
                            else:
                                responseDict = response.json()
    ...
    def dfwRulesRollback(self):
        ...
        try:
            ...
            orgVDCGroupIDList = list(self.rollback.apiData['OrgVDCGroupID'].values()) if self.rollback.apiData.get(
                'OrgVDCGroupID') else []
    ...
    def dfwGroupsRollback(self):
        ...
        try:
            ...
            # Acquiring thread lock
            self.lock.acquire(blocking=True)
            orgVDCGroupID = list(self.rollback.apiData['OrgVDCGroupID'].values()) if self.rollback.apiData.get(
                'OrgVDCGroupID') else []
            if orgVDCGroupID:
                if float(self.version) >= float(vcdConstants.API_VERSION_ANDROMEDA):
                    ...
                    for _, sublist in chunksOfList(list(groups.items()), 40):
                        ...
                        for firewallGroupName, firewallGroup in sublist:
                            deleteFirewallGroupUrl = '{}{}'.format(vcdConstants.OPEN_API_URL.format(self.ipAddress),
                                                                   vcdConstants.FIREWALL_GROUP.format(
                                                                       firewallGroup['id']))
